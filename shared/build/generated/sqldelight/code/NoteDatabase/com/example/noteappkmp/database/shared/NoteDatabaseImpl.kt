package com.example.noteappkmp.database.shared

import com.example.noteappkmp.database.NoteDatabase
...
private class NoteQueriesImpl(
  ...
  public override fun deleteNote(id: Long): Unit {
    driver.execute(-596978116, """
    |DELETE FROM noteEntity
    |WHERE id = ?
    """.trimMargin(), 1) {
      bindLong(1, id)
    }
    notifyQueries(-596978116, {database.noteQueries.getNoteById + database.noteQueries.getAllNotes})
  }
  ...
  private inner class GetNoteByIdQuery<out T : Any>(
    ...
    public override fun execute(): SqlCursor = driver.executeQuery(-825247173,
        """SELECT * FROM noteEntity WHERE id = ?""", 1) {
      bindLong(1, id)
    }
